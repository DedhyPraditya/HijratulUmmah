...
<template>
  ...
  <!-- Toggle Show All dipindah ke bawah -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="(donatur, idx) in displayedDonors" :key="donatur.id"
            ...
            <!-- Info -->
            <div class="flex-1">
                          <div class="font-large text-base"><strong>{{ donatur.nama || 'Hamba Allah' }}</strong></div>
                          <div class="text-base font-bold text-green-600">Rp {{ donatur.nominal.toLocaleString('id-ID') }}</div>
        ...
        <div class="flex justify-center mt-6">
          <button @click="toggleShowAll" class="text-green-600 font-semibold hover:underline text-sm bg-transparent border-0 cursor-pointer" type="button">
...
async function fetchDonaturTerbaru() {
  try {
    const res = await fetch('http://localhost:3000/api/donasi');
